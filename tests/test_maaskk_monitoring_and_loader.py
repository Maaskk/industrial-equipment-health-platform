...
class MaaskkMonitoringAndLoaderTests(unittest.TestCase):
    ...
    def test_load_model_refuses_missing_artifact_by_default(self):
        with tempfile.TemporaryDirectory() as directory:
            missing_path = Path(directory) / "missing-model.pkl"

            with self.assertRaises(FileNotFoundError):
                load_model(local_model_path=missing_path)

    def test_load_model_uses_fallback_only_when_explicitly_allowed(self):
        with tempfile.TemporaryDirectory() as directory:
            missing_path = Path(directory) / "missing-model.pkl"

            model = load_model(local_model_path=missing_path, allow_fallback=True)
